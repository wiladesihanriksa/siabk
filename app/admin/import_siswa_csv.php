...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file_excel'])) {
    ...
    // Validasi file
    if ($fileExt != 'xlsx') {
        header("location: kelas_siswa.php?id=".$_POST['kelas_id']."&alert=error&msg=File harus berformat Excel .xlsx");
        exit;
    }
    ...
    if (move_uploaded_file($fileTmp, $uploadFile)) {
        try {
            ...
            for ($i = 1; $i < count($data); $i++) {
                ...
                // Ambil jurusan dari kelas jika ada kelas_id (prioritas)
                $jurusan_id = null;
                if (isset($_POST['kelas_id']) && !empty($_POST['kelas_id'])) {
                    $kelas_id = mysqli_real_escape_string($koneksi, $_POST['kelas_id']);
                    $kelas_query = mysqli_query($koneksi, "SELECT kelas_jurusan FROM kelas WHERE kelas_id = '$kelas_id'");
                    if ($kelas_query && mysqli_num_rows($kelas_query) > 0) {
                        $kelas_data = mysqli_fetch_assoc($kelas_query);
                        $jurusan_id = $kelas_data['kelas_jurusan'];
                    }
                }
                
                // Jika jurusan belum didapat dari kelas, cari dari nama jurusan di Excel
                if (!$jurusan_id && !empty($jurusan_nama)) {
                ...
                // Jika masih belum ada jurusan, skip
                if (!$jurusan_id) {
                    $errorCount++;
                    $errors[] = "Baris " . ($i + 1) . ": Jurusan tidak ditemukan";
                    continue;
                }
                ...
                if ($insert_siswa) {
                    ...
                    // Tambahkan ke kelas jika ada kelas_id (otomatis masuk ke kelas yang dipilih)
                    if (isset($_POST['kelas_id']) && !empty($_POST['kelas_id'])) {
                        $kelas_id = mysqli_real_escape_string($koneksi, $_POST['kelas_id']);
                        
                        // Cek apakah siswa sudah ada di kelas ini
                        $cek_kelas = mysqli_query($koneksi, "SELECT * FROM kelas_siswa WHERE ks_siswa = '$siswa_id' AND ks_kelas = '$kelas_id'");
