<?php
/**
 * Import Data Prestasi Universal - Mendukung CSV dan Excel
 */
...
if (!isset($_FILES['file_excel']) || $_FILES['file_excel']['error'] !== UPLOAD_ERR_OK) {
    header('Location: input_prestasi.php?alert=error&msg=File tidak ditemukan atau error saat upload');
    exit;
}
...
if (!in_array($fileExtension, ['csv', 'xlsx'])) {
    header('Location: input_prestasi.php?alert=error&msg=Format file harus .csv atau .xlsx');
    exit;
}
...
if ($fileExtension === 'csv') {
    ...
    // Baca file Excel menggunakan library yang ada
    include '../library/excel_reader.php';
    $excel = new ExcelReader($file['tmp_name']);
    $excelData = $excel->read();
    
    // Konversi data Excel ke format yang sama dengan CSV
...
for ($i = 1; $i < count($data); $i++) {
    ...
    if (empty($row[0]) || (is_string($row[0]) && strpos($row[0], '===') !== false)) {
        continue;
    }
